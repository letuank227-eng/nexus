...
const TABS = [
  ...
  },
  {
    href: '/app/tasks',
    label: 'Tasks',
    match: (p: string) => p.startsWith('/app/tasks') || p === '/app/dashboard',
    icon: (a: boolean) => (
...
// memo: only re-renders when pathname changes
const BottomNav = memo(function BottomNav() {
  const pathname = usePathname()

  return (
    <nav
      className="bg-white/95 backdrop-blur-md border-t border-gray-200/80 flex justify-around items-start pt-2 fixed bottom-0 left-0 right-0 z-30"
      ...
      {TABS.map(tab => {
        ...
        return (
          <Link
            ...
            href={tab.href}
            prefetch={true}
            className={`flex flex-col items-center justify-start w-full gap-0.5 transition-colors duration-150 active:scale-95 ${
              active ? 'text-[#007AFF]' : 'text-gray-400 hover:text-gray-600'
            }`}
          >
            <div className={`w-6 h-0.5 rounded-full mb-0.5 transition-all duration-200 ${active ? 'bg-[#007AFF]' : 'bg-transparent'}`}/>
            {tab.icon(active)}
